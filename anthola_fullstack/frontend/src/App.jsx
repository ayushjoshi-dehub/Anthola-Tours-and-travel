...
function SeatGrid({ seatCount = 36, booked = [], locked = [], selected = [], onToggle, disabled }) {
      ...
      <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 lg:grid-cols-4">
        {seats.map((seat) => {
          ...
          const cls = isBooked
            ...
            : lock
              ? isMine ? 'bg-emerald-500 text-white' : 'bg-rose-500/80 text-white cursor-not-allowed'
              : isSelected ? 'bg-amber-400 text-slate-950' : 'bg-white/5 text-slate-200';
          return (
            <button
              key={seat}
              disabled={disabled || isBooked || (lock && !isMine)}
...
function PassengerPage() {
  ...
  const seatQuery = useQuery({
    queryKey: ['seat-state', routeId, filters.date],

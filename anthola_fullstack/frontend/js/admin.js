...
export function initAdminPage() {
  ...
  if (!isAdminUser(user)) {
    root.innerHTML = `
      <div class="bg-white border border-slate-200 rounded-3xl p-8">
        <h2 class="text-2xl font-extrabold">Admin access required</h2>
        <p class="mt-2 text-slate-600">Please sign in as <b>admin</b> to view this page.</p>
        <p class="mt-4 text-sm text-slate-500">Backend seeds an admin user by default: <b>admin / admin123</b>.</p>
      </div>
    `;
    return;
  }
  ...
  root.innerHTML = `
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        ...
        <p class="text-slate-600 mt-1">Manage routes, bookings, users, and messages (backend APIs).</p>
      </div>
  ...
  const render = async (key) => {
    ...
    if (key === 'users') return renderUsers(panel);
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <div class="p-6 rounded-2xl border border-slate-200 bg-slate-50">
          ...
          <ul class="mt-3 space-y-2 text-slate-700 text-sm">
            ...
            <li><i class="fa-solid fa-users text-orange-500 mr-2"></i>Users (block / unblock)</li>
        ...
        <div class="p-6 rounded-2xl border border-slate-200 bg-white">
          ...
          <p class="mt-2 text-slate-600 text-sm">Next upgrade: real-time seat locking (Socket.IO) + payment gateway.</p>
        </div>
      </div>
    `;
  }
  ...
  async function renderRoutes(mount) {
    let routes = [];
    try {
      const data = await apiFetch('/api/routes?includeInactive=1', { auth: false });
  ...
  async function exportJson() {
    try {
      const [routes, bookings, users, messages, stats] = await Promise.all([
        apiFetch('/api/routes?includeInactive=1', { auth: false }),
